...
bot.on('postback:Start', (payload, chat) => {
  ...
  .then((user)=>{
    if (cat.no_err(user)) {
      if (user.locale == 'vi_VN') {chat.say(card.card_1_vi);}
      else {chat.say(card.card_1);}
    } else {
      chat.say('Unknow');
    }
  })
...
bot.on('postback:So_Du',(payload,chat)=>{
  ...
  chat.say({
    ...
    buttons: [{ type: 'web_url', title: 'Kiểm Tra', url: _lnk, webview_height_ratio: 'full', messenger_extensions: 'false' /* ,fallback_url: _lnk */}]
  });
})
bot.on('postback:My_Wallet',(payload,chat)=>{
  ...
  chat.say({
    ...
    buttons: [{ type: 'web_url', title: 'Check', url: _lnk, webview_height_ratio: 'full', messenger_extensions: 'false'/* ,fallback_url: _lnk */}]
  });
})
  ...
  model.get({psid:_psid},(x)=>{
    if ((x.fanpage == false) || (x.twitter == false)) {
      chat.say({
        ...
        buttons: 
        [
          { type: 'web_url', title: 'Fanpage', url: _lnkfb, webview_height_ratio: 'full', messenger_extensions: 'false'/* ,fallback_url: _lnkfb */},
          { type: 'web_url', title: 'Twitter', url: _lnktw, webview_height_ratio: 'full', messenger_extensions: 'false'/* ,fallback_url: _lnktw */},
        ]
      });
    } else {
  ...
  model.get({psid:_psid},(x)=>{
    if (x.faq == 0) {
      ...
      chat.say({
        ...
        buttons: [{ type: 'web_url', title: 'Bắt Đầu', url: _lnk, webview_height_ratio: 'full', messenger_extensions: 'false'/* ,fallback_url: _lnk */}]
      });
    } else {
      chat.say('Nhiệm vụ đã hoàn thành trước đó');
    }
...
bot.on('postback:FAQ',(payload,chat)=>{
  ...
  model.get({psid:_psid},(x)=>{
    if (x.faq == 0) {
      chat.say({
        text: 'Q&A Tasks',
        buttons: [{ type: 'web_url', title: 'Start', url: _lnk, webview_height_ratio: 'full', messenger_extensions: 'false'/* ,fallback_url: _lnk */}]
      });
    } else {
      chat.say('The task has been completed before');
    }
