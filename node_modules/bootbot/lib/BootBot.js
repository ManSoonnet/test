...
class BootBot extends EventEmitter {
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || 'POST';
    return fetch(`https://graph.facebook.com/v3.2/me/${endpoint}?access_token=${this.accessToken}`, {
      method,
      headers: {
   ...
   * @returns {Promise}
   */
  getUserProfile(userId) {                //v2.12/
    const url = `https://graph.facebook.com/v3.2/${userId}?fields=first_name,last_name,locale&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
